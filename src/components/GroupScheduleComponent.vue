...
export default {
    ...
    mounted() {
        this.sortedSchedules = this.sortByDate();
    },
    methods: {
        ...
        sortByDate(): MatchInfo[] {
            ...
            sorted.sort((a, b) => {
                const dateA = a.schedule ? new Date(`${a.schedule.date} ${a.schedule.time}`) : null;
                const dateB = b.schedule ? new Date(`${b.schedule.date} ${b.schedule.time}`) : null;

                if (dateA === null && dateB === null) {
                    return 0; // Both dates are null, consider them equal
                } else if (dateA === null) {
                    return 1; // dateA is null, so it comes after dateB
                } else if (dateB === null) {
                    return -1; // dateB is null, so it comes after dateA
                } else {
                    return dateA.getTime() - dateB.getTime(); // Compare the valid dates
                }
            });
...
<template>
    <div class="group-schedule w-full truncate rounded-lg">
        <table v-if="sort" class="w-full">
            <tr>
                <th class="w-[5%] truncate-text">ID</th>
                <th class="w-[10%] truncate-text">DATE</th>
                <th class="w-[5%] truncate-text">TIME</th>
                <th class="w-[25%] truncate-text">RED TEAM</th>
                <th class="w-[5%] truncate-text">SCORE</th>
                <th class="w-[25%] truncate-text">BLUE TEAM</th>
                <th class="w-[15%] truncate-text">REFEREE</th>
                <th class="w-[5%] truncate-text">
